...
export default function ReportsPage() {
  ...
  payments.forEach((p: any) => {
    ...
    methodBreakdown[method] = (methodBreakdown[method] || 0) + Number(p.paidAmount || 0);
  });
  ...
  transactions.filter((t: any) => t.type === "income").forEach((t: any) => {
    incomeByCategory[t.category] = (incomeByCategory[t.category] || 0) + Number(t.amount || 0);
  });
  ...
  function printFullReport() {
    ...
    const schoolOverview = showSchoolMetrics ? `
      <div class="stat-grid">
    ...
    const academicSections = showSchoolMetrics ? `
      <div class="section">
    ...
    const html = `
      <div class="header">
        ...
        <div style="text-align:right"><div style="font-size:16px;font-weight:700">${showSchoolMetrics ? "FINANCIAL & SCHOOL SUMMARY" : "FINANCIAL SUMMARY"}</div><div style="font-size:12px;color:#6b7280">Report Date: ${new Date().toLocaleDateString("en-KE")}</div></div>
      </div>
      ...
      <div class="section">
        <div class="section-title">Fee Collections & Payroll</div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px">
          <div>
            <div style="font-size:13px;font-weight:600;margin-bottom:8px">By Payment Method</div>
            <table><thead><tr><th>Method</th><th>Amount Received</th></tr></thead><tbody>${paymentMethodRows || '<tr><td colspan="2" style="color:#6b7280">No data</td></tr>'}</tbody>
          ...
          <div>
            ...
            <table><thead><tr><th>Metric</th><th>Value</th></tr></thead><tbody>
              <tr><td>Paid Payroll Records</td><td>${payrollList.filter((p: any) => p.status === "paid").length} / ${payrollList.length}</td></tr>
    ...
    printHTML(html, `${SCHOOL_NAME} - ${showSchoolMetrics ? "Full School Report" : "Financial Report"}`);
  }
